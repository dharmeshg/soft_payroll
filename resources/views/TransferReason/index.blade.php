...
<div class="page-wrapper">
        ...
        <div class="container-fluid">
          ...
          <div class="row">
            <div class="col-12">
              <div class="card">
                <div class="card-body">
                <div class="" style="display: flex; justify-content: space-between; padding-bottom: 20px;">
                  <h5 class="card-title">Transfer Reason List</h5>
                  <div class="">
                    <a href="{{ route('add.transferreason')}}" class="next-page">Add Transfer Reason</a>
                  </div>
                  </div>
                  <div class="table-responsive">
                    ...
                    >
                      ...
                      <tbody>
                          @foreach($data as $key=>$transferreason)
                          <tr>
                            <th>{{$key+1}}</th>                            
                            <td>{{$transferreason->transferreason}}</td>
                            <td>{{$transferreason->description}}</td> 
                            <td>@if($transferreason->id != 1)
                            <input type="checkbox" class="toggle" id="rounded{{ $transferreason->id }}" data-itemid="{{ $transferreason->id }}" {{ ( ( $transferreason->status == 1 ) ? 'checked' : '' ) }}>
                            <label for="rounded{{ $transferreason->id }}" data-checked="ON" data-unchecked="off" class="rounded on-off-btn"></label>
                            @else
                            -
                            @endif  
                            <a href="{{route('transferreason.edit',[$transferreason->id])}}" class="fas fa-edit"></a>
                            <a href="{{route('transferreason.delete',[$transferreason->id])}}" class="fas fa-trash delete"></a>
                          </tr>
                         @endforeach
                      </tbody>
                    </table>
     ...
     @section('script')
      <script>
        ...
        var data_transferreasonid = $(this).attr( 'data-itemid' );
        var data_status = ( ( $(this).prop("checked") == true ) ? '1' : '0' );
        $.ajax({
            type: "POST",
            url: '{{ route("transferreason.statusupdate") }}',
            data: {
                data_transferreasonid: data_transferreasonid,
