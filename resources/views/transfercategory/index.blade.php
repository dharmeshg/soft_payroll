...
<div class="page-wrapper">
        ...
        <div class="container-fluid">
          ...
          <div class="row">
            <div class="col-12">
              <div class="card">
                <div class="card-body"> 
                  <h5 class="card-title">Transfer Category List</h5>   
                  <div class="table-responsive">
                    ...
                    >
                      <thead>
                        <tr>
                          <th>Sr.No</th>
                          <th>Transfer Category Name</th>
                          <th>Description</th>
                        </tr>
                      </thead>
                      <tbody>
                          @foreach($data as $key=>$transfercategory)
                          <tr>
                            <th>{{$key+1}}</th>                            
                            <td>{{$transfercategory->categoryname}}</td>
                            <td>{{$transfercategory->description}}</td> 
                          </tr>
                         @endforeach
                      </tbody>
                    </table>
     ...
     @section('script')
      <script>
        ...
        var data_transfercategoryid = $(this).attr( 'data-itemid' );
        var data_status = ( ( $(this).prop("checked") == true ) ? '1' : '0' );
        $.ajax({
            type: "POST",
            url: '{{ route("transfercategory.statusupdate") }}',
            data: {
                data_transfercategoryid: data_transfercategoryid,
